...
const Field = ({ label, value }: { label: string; value: string }) => <Box sx={{ borderBottom: { xs: '1px solid #e6ece8', sm: 0 }, py: { xs: 1.15, sm: 0 } }}><Typography color="text.secondary" sx={{ fontSize: '.68rem', fontWeight: 650, letterSpacing: '.045em', textTransform: 'uppercase' }}>{label}</Typography><Typography sx={{ fontSize: '.9rem', fontWeight: 620, mt: .35 }}>{value || '—'}</Typography></Box>
...
export const EmployeeDetailsPage = () => {
  ...
  return <Stack spacing={2.5}>
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ alignItems: { sm: 'center' }, justifyContent: 'space-between' }}>
      <Box><Button color="inherit" onClick={() => navigate('/employees')} size="small" startIcon={<BackIcon />} sx={{ color: 'text.secondary', fontSize: '.74rem', px: .2 }}>Back to employees</Button><Typography component="h1" sx={{ fontWeight: 750, letterSpacing: '-.03em', mt: { xs: 1.25, sm: .25 } }} variant="h4">Employee details</Typography></Box>
      <Stack direction="row" spacing={{ xs: .6, sm: 1 }} sx={{ flexWrap: { xs: 'nowrap', sm: 'wrap' }, width: { xs: '100%', sm: 'auto' } }}>{isSelf && currentEmployee?.role === 'EMPLOYEE' ? <Button onClick={() => navigate('/profile')} startIcon={<EditIcon />} variant="contained">Edit my profile</Button> : <>{canManage && <Button onClick={() => navigate(`/employees/${employee.id}/edit`)} startIcon={<EditIcon />} sx={{ flex: { xs: 1, sm: 'initial' }, fontSize: { xs: '.7rem', sm: '.875rem' }, minWidth: 0, px: { xs: .6, sm: 2.35 }, '& .MuiButton-startIcon': { mr: { xs: .35, sm: 1 } }, '& .MuiSvgIcon-root': { fontSize: { xs: 16, sm: 18 } } }} variant="contained"><Box component="span" sx={{ display: { xs: 'inline', sm: 'none' } }}>Edit</Box><Box component="span" sx={{ display: { xs: 'none', sm: 'inline' } }}>Edit employee</Box></Button>}{canDisable && <Button color="warning" onClick={() => setConfirmAction('disable')} startIcon={<DisableIcon />} sx={{ flex: { xs: 1, sm: 'initial' }, fontSize: { xs: '.7rem', sm: '.875rem' }, minWidth: 0, px: { xs: .5, sm: 1.5 }, '& .MuiButton-startIcon': { mr: { xs: .3, sm: 1 } }, '& .MuiSvgIcon-root': { fontSize: { xs: 16, sm: 18 } } }} variant="outlined">Disable</Button>}{canDelete && <Button color="error" onClick={() => setConfirmAction('delete')} startIcon={<DeleteIcon />} sx={{ flex: { xs: 1, sm: 'initial' }, fontSize: { xs: '.7rem', sm: '.875rem' }, minWidth: 0, px: { xs: .5, sm: 1.5 }, '& .MuiButton-startIcon': { mr: { xs: .3, sm: 1 } }, '& .MuiSvgIcon-root': { fontSize: { xs: 16, sm: 18 } } }} variant="outlined">Delete</Button>}</>}</Stack>
    </Stack>

    {hrBlocked && <Alert severity="info">HR Managers can view this Super Admin profile but cannot modify it.</Alert>}
    <Paper elevation={0} sx={{ background: 'linear-gradient(120deg,#173d29 0%,#2f7045 65%,#4c8960 100%)', borderRadius: 3, boxShadow: '0 13px 30px rgba(27,67,41,.17)', color: '#fff', p: { xs: 1.35, md: 2.2 } }}><Stack direction="row" spacing={{ xs: 1.15, sm: 1.7 }} sx={{ alignItems: 'center' }}><Avatar src={employee.profileImageUrl ?? undefined} sx={{ bgcolor: 'rgba(255,255,255,.18)', border: '2px solid rgba(255,255,255,.3)', fontSize: { xs: '1rem', sm: '1.2rem' }, fontWeight: 750, height: { xs: 48, sm: 60 }, width: { xs: 48, sm: 60 } }}>{initials(employee.name)}</Avatar><Box sx={{ flex: 1 }}><Typography sx={{ fontSize: { xs: '1.05rem', sm: '1.3rem' }, fontWeight: 760 }}>{employee.name}</Typography><Typography sx={{ color: 'rgba(255,255,255,.78)', fontSize: { xs: '.72rem', sm: '.88rem' }, mt: .1 }}>{employee.designation} · {employee.department.name}</Typography><Stack direction="row" spacing={{ xs: .45, sm: .8 }} sx={{ mt: { xs: .5, sm: .75 } }}><Chip label={roleLabel(employee.role)} size="small" sx={{ bgcolor: 'rgba(255,255,255,.15)', color: '#fff', fontSize: { xs: '.62rem', sm: '.8125rem' }, height: { xs: 21, sm: 24 } }} /><Chip label={employee.status === 'ACTIVE' ? '● Active' : '● Inactive'} size="small" sx={{ bgcolor: employee.status === 'ACTIVE' ? 'rgba(192,240,205,.18)' : 'rgba(255,190,190,.18)', color: '#fff', fontSize: { xs: '.62rem', sm: '.8125rem' }, height: { xs: 21, sm: 24 } }} /></Stack></Box><Box sx={{ ml: 'auto', textAlign: 'right' }}><Typography sx={{ color: 'rgba(255,255,255,.63)', fontSize: '.64rem', textTransform: 'uppercase' }}>Employee ID</Typography><Typography sx={{ fontSize: { xs: '.78rem', sm: '.92rem' }, fontWeight: 720, mt: .2 }}>{employee.employeeId}</Typography></Box></Stack></Paper>

    <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', lg: 'minmax(0,1.1fr) minmax(0,.9fr)' } }}>
      <Paper elevation={0} sx={detailsCardSx}><SectionTitle icon={<InfoIcon />} subtitle="Personal and workplace record" title="Employee information" /><Box sx={{ display: 'grid', gap: { xs: 0, sm: 2.5 }, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,minmax(0,1fr))' }, p: { xs: '0 1.7rem .35rem', sm: 2.3 }, '& > div:last-of-type': { borderBottom: 0 } }}><Field label="Full name" value={employee.name} /><Field label="Work email" value={employee.email} /><Field label="Phone" value={employee.phone ?? 'Not provided'} /><Field label="Department" value={employee.department.name} /><Field label="Designation" value={employee.designation} /><Field label="Annual salary" value={currency(employee.salary)} /><Field label="Joining date" value={date(employee.joiningDate)} /><Field label="Role" value={roleLabel(employee.role)} /></Box></Paper>
      <Stack spacing={2} sx={{ '& > .MuiPaper-root': { transition: 'border-color 180ms ease, box-shadow 180ms ease, transform 180ms ease' }, '& > .MuiPaper-root:hover': { borderColor: '#ccdacf', boxShadow: '0 13px 28px rgba(30,65,42,.105)', transform: 'translateY(-2px)' } }}>
        <Paper elevation={0} sx={{ border: '1px solid #dfe7e1', borderRadius: 2.5, boxShadow: '0 8px 22px rgba(30,65,42,.06)', overflow: 'hidden' }}><SectionTitle icon={<ManagerIcon />} subtitle="Current reporting relationship" title="Reporting manager" /><Box sx={{ p: employee.reportingManager ? 2.2 : 1.5 }}>{employee.reportingManager ? <Stack direction="row" spacing={1.3} sx={{ alignItems: 'center' }}><Avatar sx={{ bgcolor: '#e3efe7', color: '#39704a', fontSize: '.85rem', fontWeight: 700 }}>{initials(employee.reportingManager.name)}</Avatar><Box sx={{ flex: 1 }}><Typography sx={{ fontWeight: 700 }}>{employee.reportingManager.name}</Typography><Typography color="text.secondary" variant="body2">{employee.reportingManager.employeeId}</Typography></Box><Button endIcon={<ArrowIcon />} onClick={() => navigate(`/employees/${employee.reportingManager!.id}`)} size="small">View</Button></Stack> : <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.3} sx={{ alignItems: { sm: 'center' }, bgcolor: '#f5f9f6', border: '1px dashed #cbdacf', borderRadius: 2, px: 1.5, py: 1.4 }}><Box sx={{ alignItems: 'center', bgcolor: '#dfede4', borderRadius: '50%', color: '#4b805d', display: 'flex', flexShrink: 0, height: 42, justifyContent: 'center', width: 42 }}><ManagerIcon /></Box><Box sx={{ flex: 1 }}><Typography sx={{ color: '#304c39', fontSize: '.84rem', fontWeight: 720 }}>Top-level reporting position</Typography><Typography color="text.secondary" sx={{ fontSize: '.7rem', mt: .25 }}>No manager is currently assigned to this employee.</Typography></Box>{canManage && <Button onClick={() => navigate(`/employees/${employee.id}/edit`)} size="small" startIcon={<EditIcon />} variant="outlined">Assign manager</Button>}</Stack>}</Box></Paper>
