...
const detailIcons = {
  name: 'M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4Zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4Z',
  email: 'M20 4H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2Zm0 4-8 5-8-5V6l8 5 8-5v2Z',
  phone: 'M6.62 10.79a15.46 15.46 0 0 0 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20a1 1 0 0 1-1 1C10.61 21 3 13.39 3 4a1 1 0 0 1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2Z',
...
const Detail = ({ icon, mobileIcon, label, value }: { icon?: string; mobileIcon?: string; label: string; value: string }) => <Stack direction="row" spacing={1.1} sx={{ alignItems: 'flex-start', bgcolor: { xs: '#f9fbf9', sm: 'transparent' }, border: { xs: '1px solid #e3eae5', sm: 0 }, borderRadius: { xs: 1.75, sm: 0 }, p: { xs: 1.2, sm: 0 } }}>{(icon || mobileIcon) && <Box sx={{ bgcolor: '#e8f1eb', borderRadius: 1.3, color: 'primary.main', display: mobileIcon && !icon ? { xs: 'flex', sm: 'none' } : 'flex', p: .65 }}><SvgIcon sx={{ fontSize: 17 }}><path d={icon ?? mobileIcon} /></SvgIcon></Box>}<Box sx={{ minWidth: 0 }}><Typography color="text.secondary" sx={{ fontSize: '.68rem', fontWeight: 650, letterSpacing: '.04em', textTransform: 'uppercase' }}>{label}</Typography><Typography sx={{ fontSize: '.92rem', fontWeight: 650, mt: .3, overflowWrap: 'anywhere' }}>{value || '—'}</Typography></Box></Stack>
...
export const ProfilePage = () => {
  ...
  return <Stack component="form" onSubmit={save} spacing={2.5}>
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ alignItems: { sm: 'flex-end' }, justifyContent: 'space-between' }}><Box><Typography component="h1" sx={{ fontWeight: 750, letterSpacing: '-.03em' }} variant="h4">My profile</Typography><Typography color="text.secondary" sx={{ mt: .4 }}>Manage your personal details and view your Playstack employment record.</Typography></Box>{!editing && <Button onClick={() => setEditing(true)} startIcon={<EditIcon />} sx={{ boxShadow: '0 5px 13px rgba(47,112,69,.22)' }} variant="contained">Edit profile</Button>}</Stack>
    {error && <Alert severity="error">{error}</Alert>}
    <Paper elevation={0} sx={{ background: 'linear-gradient(120deg,#173d29 0%,#2f7045 65%,#4c8960 100%)', borderRadius: 3, boxShadow: '0 12px 28px rgba(27,67,41,.17)', color: '#fff', overflow: 'hidden', p: { xs: 1.35, md: 2.2 } }}><Stack direction="row" spacing={{ xs: 1.15, sm: 1.8 }} sx={{ alignItems: 'center' }}><Avatar src={profileImageUrl || undefined} sx={{ bgcolor: 'rgba(255,255,255,.18)', border: '2px solid rgba(255,255,255,.3)', fontSize: { xs: '1rem', sm: '1.2rem' }, fontWeight: 750, height: { xs: 48, sm: 60 }, width: { xs: 48, sm: 60 } }}>{initials(name)}</Avatar><Box sx={{ flex: 1 }}><Typography sx={{ fontSize: { xs: '1.05rem', sm: '1.3rem' }, fontWeight: 760 }}>{employee.name}</Typography><Typography sx={{ color: 'rgba(255,255,255,.78)', fontSize: { xs: '.72rem', sm: '.88rem' }, mt: .1 }}>{employee.designation} · {employee.department.name}</Typography><Stack direction="row" spacing={{ xs: .45, sm: .8 }} sx={{ mt: { xs: .5, sm: .8 } }}><Chip label={roleLabel(employee.role)} size="small" sx={{ bgcolor: 'rgba(255,255,255,.15)', color: '#fff', fontSize: { xs: '.62rem', sm: '.8125rem' }, height: { xs: 21, sm: 24 } }} /><Chip label={employee.status === 'ACTIVE' ? '● Active' : '● Inactive'} size="small" sx={{ bgcolor: 'rgba(255,255,255,.15)', color: '#fff', fontSize: { xs: '.62rem', sm: '.8125rem' }, height: { xs: 21, sm: 24 } }} /></Stack></Box><Box sx={{ ml: 'auto', textAlign: 'right' }}><Typography sx={{ color: 'rgba(255,255,255,.65)', fontSize: '.65rem', textTransform: 'uppercase' }}>Employee ID</Typography><Typography sx={{ fontSize: { xs: '.78rem', sm: '.94rem' }, fontWeight: 700, mt: .2 }}>{employee.employeeId}</Typography></Box></Stack></Paper>
    <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', lg: 'minmax(0,1.05fr) minmax(0,.95fr)' } }}>
      <Paper elevation={0} sx={{ border: '1px solid #dfe7e1', borderRadius: 2.5, boxShadow: '0 8px 22px rgba(30,65,42,.06)', overflow: 'hidden' }}>
        ...
        <Stack spacing={{ xs: 1.2, sm: 2.1 }} sx={{ p: { xs: 1.4, sm: 2.3 } }}>
          {editing ? (
            <>
